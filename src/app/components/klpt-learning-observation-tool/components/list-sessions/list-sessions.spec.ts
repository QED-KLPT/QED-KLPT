...
describe('ListSessions', () => {
  describe('with sessions', () => {
    ...
    it('should render clickable rows with routerLink based on pageIndex', () => {
      ...
      expect(hrefs.some((h) => h?.includes('/learning-observation-tool/select-domains/session-1'))).toBe(true);
      expect(hrefs.some((h) => h?.includes('/learning-observation-tool/select-behaviours/session-2'))).toBe(true);
    });
